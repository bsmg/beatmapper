...
```ts title="src/components/GlobalShortcuts/GlobalShortcuts.tsx"
switch (ev.code) {
  case 'Space': {
    ...
  }
...
First, let's find where the shortcuts live now. They're in `src/components/Events/KeyboardShortcuts.tsx`:

```ts title="src/components/Events/KeyboardShortcuts.tsx"
case 'Digit1': {
