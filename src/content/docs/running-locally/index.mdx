...
We'll also need to generate some files via the `prepare` scripts that have been predefined in the `package.json` file.


```sh
yarn prepare:panda && yarn prepare:tsr
```
...
- Shortcuts used across multiple views are registered in `src/components/app/templates/shortcuts/default.tsx`
- Shortcuts exclusive to the "Notes view" are registered in `src/components/app/templates/shortcuts/notes.tsx`
- Shortcuts exclusive to the "Events view" are registered in `src/components/app/templates/shortcuts/events.tsx`
...
Beatmapper uses the standard convention of having <Shortcut>META+Z</Shortcut> used for undo. On QWERTY keyboards, this works great, but on AZERTY keyboards, this shortcut closes the browser tab!

We want to remap this to <Shortcut>META+W</Shortcut> instead

Inside `GlobalShortcuts.tsx`, there is a section of code around line 90 that looks like:

```ts title="src/components/app/templates/shortcuts/default.tsx"
switch (ev.code) {
  case 'Space': {
    ...
  }
...
To briefly explain what's happening here: this code runs whenever a key on the keyboard is pressed, and each `case` is listening for a different letter. When the <Shortcut>Spacebar</Shortcut> is pressed, the code inside `case 'Space'` is run.

For individual letters, the pattern is `KeyX`. So the <Shortcut>A</Shortcut> key is `KeyA`. Scroll down until you see this code:
...
In the events view, you can use the numbers 1 through 4 to pick between the different event types: <Shortcut>1</Shortcut> is "On", <Shortcut>2</Shortcut> is "Off", <Shortcut>3</Shortcut> is "Flash", <Shortcut>4</Shortcut> is "Fade".

Let's say we want to remap this to use WASD, but only when the "Shift" key is held.

First, let's find where the shortcuts live now. They're in `src/components/app/templates/shortcuts/events.tsx`:

```ts title="src/components/app/templates/shortcuts/events.tsx"
case 'Digit1': {
...
Let's say we want <Shortcut>Shift + A</Shortcut> to trigger the "On" event. Your instinct might be to simply update `Digit1` to `KeyA`:
...
That will allow the <Shortcut>A</Shortcut> key to toggle this tool, but remember, we want it to be <Shortcut>Shift + A</Shortcut>, not just <Shortcut>A</Shortcut> on its own!
...
There's one last problem we need to tackle, though; it turns out, we _already had_ a keybinding for the <Shortcut>A</Shortcut> key. If we run this code as-is, we'll _overwrite_ some other bindings that exist for this key.

In VS Code, if you type <Shortcut>META+F</Shortcut>, you'll pull up a little search text field. Enter `KeyA`, and you'll discover that this code exists in the same file:
...
- When <Shortcut>META+A</Shortcut> is pressed, select all events in the view.
- When <Shortcut>A</Shortcut> is pressed on its own, switch the edit mode (<Shortcut>S</Shortcut> is for "Add" mode, <Shortcut>S</Shortcut> is for "Select" mode).
...
- If the <Shortcut>Shift</Shortcut> key is held, select the "On" event
- If the <Shortcut>META</Shortcut> key is held, select all entities
