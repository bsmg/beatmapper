...
- You can now quick-edit the duration of an obstacle using <Shortcut>option</Shortcut> + scroll, which now replaces the old janky drag controls from before.
...
since many of these changes are already documented in extensive detail within the many [PRs](https://github.com/bsmg/beatmapper/pulls?q=is%3Apr+is%3Aclosed) 
...
- The manifest has been updated according to the new compatibility baselines, supporting maskable icons and additional metadata.
...
- The beatmap color scheme is now respected, and will derive the correct values from the base environment scheme, 
...
- Full-height obstacles have smarter placement controls, where it is now physically impossible to place a 3-wide wall. Who needs the mapping wiki when you can just fix the problem in the first place!
...
- When the "bloom" effect is disabled, any light materials will now be tinted with their respective color to improve visibility.
  ([#1](https://github.com/bsmg/beatmapper/issues/1))
...
- The "Fast Walls" toggle has been removed, as the property was removed when moving to the new baetmap wrappers. 
  However, you can still create a fast wall by selecting an obstacle and setting the duration to a negative number if you so desire.
...
- To Kival Evan, for helping me land many changes to the `bsmap` engine that made my life significantly easier.
...
- To any users who DM'd me bug reports within the last few months or were courteous enough to drop a ping for other users. 
  Happy to say that nearly all of the reports are finally addressed in this update, and I'm glad to know that people appreciate the work that went into it.
- To you, for reading through this wall of text and may end up actually using this editor again. 
  It's honestly sick to be able to bring this project back to life, and I'm sure there will be lots of cool things to look forward to.

That's all for now. Welcome to the new era. Enjoy the update! ❤️