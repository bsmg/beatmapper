...
You can **click and drag** to dynamically select a note direction. This is my personal favourite way to place notes:

![](../../../media/images/drag-direction.gif)

Finally, you can hold down <Shortcut>ALT</Shortcut> and scroll up/down to alter the angle offset of the note in 15-degree increments:

![](../../../media/images/alter-note-angle.gif)
...
> [!warning]
> It's worth noting that full support for free obstacles will only be available for the latest map format (v4 and above).
> For legacy formats that don't offer the necessary support for this feature, Mapping Extensions will be used to allow your obstacles to appear with the correct position and scale.
> This also means that **Mapping Extensions will be required for viewing free obstacles in legacy map formats.**
...
You can resize them after placing the obstacle by holding down <Shortcut>ALT</Shortcut> and scrolling up/down:
...
As a convenience, you can also enter a single number with a modifier (eg. `+64` or`-64`) to select _everything starting or ending from_ a given beat number.

> [!note]
> When you provide a range, it will _include_ any notes on the start beat, but _exclude_ any notes on the end beat. 
> While this behaviour might be a bit confusing at first, it's **much** easier to work with, since it means you can copy/paste selections end-to-end without overlaps.
...
> [!warning]
> The tick sound may be slightly off-beat; 
> I believe this is true for all editors, because having the audio and video perfectly synchronized is a devilishly hard challenge. 
> 
