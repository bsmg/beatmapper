...
In Beatmapper's "Lightshow" view, every event type gets its own **track**. A track is a horizontal row that displays the events over a given period of time. 
For example, the "Ring Rotation" track represents every event as a green vertical bar.

The "Lightshow" view collects all environments tracks in a **table**. The table shows a specific **window** of time, measured in beats. 
...
Beatmapper supports a variety of event types, and they're a little distinct in how they function.
...
!['On', 'Flash', and 'Transition' events leave the light on, so they create a background box afterwards.](../../../media/images/light-event-types.png)
...
![](../../../media/images/boost-event-track.png)

**Color Boost** tracks will allow you to swap between two distinct sets of light colors, each of which are derived from your active color scheme.

A value of 1 will enable the color boost effect, whereas a value of 0 will disable it.

When the color boost effect is active, any light events placed after that point will change into their respective boost colors. 
This effect will also be reflected in the interpolations for background boxes.

> [!tip]
> For environments that do not support boost colors (or simply do not have them defined in their color scheme), these events will not produce a noticeable effect.
> However, players will still be able to experience any color boost effects for your map if they override the map's chosen environment or color scheme with one that has defined boost colors.
...
- For Color Boost tracks, the height will determine whether the color boost effect is enabled.
...
If you want to un-select all events, you can press the <Shortcut>escape</Shortcut> key. You can also select all _visible_ events by using <Shortcut>mod+a</Shortcut>.
...
Certain environments feature symmetrical components, such as left and right lasers, where you may want to operate on these tracks at the same time. 
After all, it can be tedious recreating laser events and speeds twice for each side.

The "laser lock" will colorize every track based on whether it has an associated symmetrical neighbor, 
and any actions you make in one track will be copied to its corresponding symmetrical track.

![](../../../media/images/event-mirror-lock.png)

You can toggle it on or off with the <Shortcut>x</Shortcut> key.

## Track Visibility

By default, Beatmapper displays all supported event tracks for a particular environment, but in some cases you may want to limit the number of visible tracks to a subset of what's available, 
either to improve performance in-editor or help with clarity on environments that feature a substantial number of tracks.

The "Track Visibility" button next to the timeline is our solution for this problem, where you can toggle the visibility of each available basic event track within the editor view:

![](../../../media/images/track-visibility.png)

You can toggle each track individually, or alternatively use the buttons at the top to filter specific subsets of tracks based on their role within the environment.
