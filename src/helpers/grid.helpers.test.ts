...
describe("Grid helpers", () => {
	describe("convertGridRow", () => {
		it("converts the first row in a 4x4 grid (1 extra row)", () => {
			...
			const actualPosition = convertGridIndicesToNaturalGrid(position[0], numCols, DEFAULT_COL_WIDTH, position[1], numRows, DEFAULT_ROW_HEIGHT);
		...
		it("Handles a row which is half-height", () => {
			...
			const actualPosition = convertGridIndicesToNaturalGrid(position[0], numCols, DEFAULT_COL_WIDTH, position[1], numRows, rowHeight);
	...
	describe("convertGridIndicesToNaturalGrid", () => {
		...
		it("handles a 4x3 grid with half-width columns", () => {
			...
			const actualPosition = convertGridIndicesToNaturalGrid(colIndex, numCols, colWidth, rowIndex, numRows, rowHeight);
		...
		it("always produces an identical value for the middle column", () => {
			...
			const [narrowX] = convertGridIndicesToNaturalGrid(colIndex, numCols, 0.5, rowIndex, numRows, rowHeight);

			const [wideX] = convertGridIndicesToNaturalGrid(colIndex, numCols, 2.5, rowIndex, numRows, rowHeight);
		...
		it("always produces an identical value for the bottom row", () => {
			...
			const [, narrowY] = convertGridIndicesToNaturalGrid(colIndex, numCols, colWidth, rowIndex, numRows, 0.5);

			const [, wideY] = convertGridIndicesToNaturalGrid(colIndex, numCols, colWidth, rowIndex, numRows, 3);
