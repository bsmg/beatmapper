...
type Serials = [...(AnyObject | undefined)[]];
type InferSerial<TSerials extends Serials, TVersion extends number> = TVersion extends 0 ? never : [_: never, ...TSerials][TVersion] extends undefined ? never : [_: never, ...TSerials][TVersion];

type SerializationOptions = [shared: AnyObject | unknown, ...(AnyObject | unknown)[]];
...
interface SerializationFactoryConfiguration<TVersion extends ImplicitVersion, TWrapper extends AnyObject | undefined, TSerials extends Serials, TSerializationOptions extends SerializationOptions, TDeserializationOptions extends SerializationOptions> {
	...
	container: {
		serialize: (data: TWrapper, options: Partial<Merge<TSerializationOptions[0], TSerializationOptions[TVersion]>>) => InferSerial<TSerials, TVersion>;
		deserialize: (data: InferSerial<TSerials, TVersion>, options: Partial<Merge<TDeserializationOptions[0], TDeserializationOptions[TVersion]>>) => TWrapper;
	};
...
export function createSerializationFactory<TWrapper extends AnyObject | undefined, TSerials extends Serials, TSerializationOptions extends SerializationOptions = [AnyObject], TDeserializationOptions extends SerializationOptions = [AnyObject]>(
	name: string,
	builder: () => SerializationFactoryOptions<TWrapper, TSerials, TSerializationOptions, TDeserializationOptions>,
) {
	const entries = builder();

	function serialize<TVersion extends ImplicitVersion>(version: TVersion, data: TWrapper, options: Partial<Merge<TSerializationOptions[0], TSerializationOptions[TVersion]>> = {}): InferSerial<TSerials, TVersion> {
	...
	function deserialize<TVersion extends ImplicitVersion>(version: TVersion, data: InferSerial<TSerials, TVersion>, options: Partial<Merge<TDeserializationOptions[0], TDeserializationOptions[TVersion]>> = {}) {
		if (!entries[version]) throw new Error(`No configuration found for version ${version}`);
