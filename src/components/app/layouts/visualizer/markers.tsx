...
interface Props {
	markers: App.Bookmark[];
	duration: number;
	offset?: number;
	onMarkerClick: (event: MouseEvent<HTMLButtonElement>, beatNum: number) => void;
	children: (marker: App.Bookmark, ctx: { offset: number; onMarkerClick: (event: MouseEvent<HTMLButtonElement>, beatNum: number) => void }) => ReactNode;
}
function AudioVisualizerMarkers({ markers, duration, offset, onMarkerClick, children }: Props) {
	...
	return [...markers].reverse().map((bookmark) => {
		const beatNumWithOffset = bookmark.beatNum + (offset ?? 0);
