...
import { APP_TOASTER, DIFFICULTY_COLLECTION, createBeatmapCharacteristicListCollection, createBeatmapDifficultyListCollection } from "$/components/app/constants";
...
function CreateBeatmapForm({ dialog, sid, bid, onSubmit: afterCreate, children }: Props) {
	...
	const Form = useAppForm({
		...
		onSubmit: async ({ value }) => {
			...
			if (withMatchingCharacteristic.length >= DIFFICULTY_COLLECTION.size) {
