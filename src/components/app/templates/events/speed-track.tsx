import { type PointerEventHandler, useCallback, useMemo, useState } from "react";
...
import { selectAllBasicEventsForTrackInWindow, selectDurationInBeats, selectEventEditorEditMode, selectEventEditorSelectedBeat, selectEventEditorStartAndEndBeat, selectEventEditorToggleMirror, selectOffsetInBeats, selectValueForTrackAtBeat } from "$/store/selectors";
import { type App, EventEditMode, type SongId } from "$/types";
import { clamp, normalize, range } from "$/utils";
import { getYForSpeed } from "./speed-track.helpers";

import { styled } from "$:styled-system/jsx";
import SpeedTrackEvent from "./speed-event";
...
function SpeedTrack({ sid, trackId, width, height, disabled }: Props) {
	...
	const endSpeed = useAppSelector((state) => selectValueForTrackAtBeat(state, { trackId, beforeBeat: endBeat }));
	...
	const handlePointerDown = useCallback<PointerEventHandler>(
		...
		[cursorAtBeat, duration, offsetInBeats, tentativeEvent, disabled, selectedEditMode],
	);
	...
	const plottablePoints = useMemo(() => {
		...
		for (const event of events) {
			...
			const x = normalize(event.beatNum, startBeat, endBeat, 0, width);
		...
		return points;
	}, [events, startBeat, endBeat, height, width, startSpeed, endSpeed]);

	return (
		<Wrapper ref={ref} style={{ height }} data-disabled={disabled} onPointerDown={handlePointerDown} onContextMenu={(ev) => ev.preventDefault()}>
			<svg width={width} height={height}>
				...
				{/* The fill for our graph area, showing easily where the current speed is at. */}
				<polyline points={plottablePoints.reduce((acc, point) => `${acc} ${point.x},${point.y}`, "")} stroke="white" strokeWidth="0.2" fill={token.var("colors.green.500")} opacity={0.5} />
				...
				{events.map((event) => (
					<SpeedTrackEvent key={event.id} sid={sid} event={event} trackId={trackId} parentWidth={width} parentHeight={height} />
				))}
				{tentativeEvent.visible && <SpeedTrackEvent sid={sid} event={tentativeEvent as App.IBasicValueEvent} trackId={trackId} parentWidth={width} parentHeight={height} />}
			</svg>
		</Wrapper>
	);
}
