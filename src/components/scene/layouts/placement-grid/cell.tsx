...
function PlacementGridCell({ rowIndex, colIndex, grid, ...rest }: CellProps) {
	...
	const handlePointerDown = useCallback(
		(ev: ThreeEvent<PointerEvent>) => {
			ev.stopPropagation();
			if (onCellPointerDown) onCellPointerDown(ev, { cellDownAt: { rowIndex, colIndex } });
		},
	...
	const handlePointerOut = useCallback(
		(ev: ThreeEvent<PointerEvent>) => {
			ev.stopPropagation();
			if (onCellPointerOut) onCellPointerOut(ev, { cellOverAt: { rowIndex, colIndex } });
		},
		[colIndex, rowIndex, onCellPointerOut],
	);

	const handleWheel = useCallback(
		(ev: ThreeEvent<WheelEvent>) => {
			ev.stopPropagation();
			if (onCellWheel) onCellWheel(ev, { cellOverAt: { rowIndex, colIndex } });
		},
		[colIndex, rowIndex, onCellWheel],
	);
